...
class Enemy {
    ...
    update(dt) {
        ...
        // Reach water and enemy starts moving from left again
        if (this.x > 505) {
            this.x = -100;
        ...
        if (player.x < this.x + 60 &&
            ...
            if (lives === 0) {
                ...
                confirm(`No lives left, Do you want to play again?`);
                lives = 3;
                gameScore = 0;
                livesLeft.innerText = lives;
                score.innerText = '';
...
class Player {
    ...
    update() {
        // Restricts Player from moving off the left/right side of canvas
        if (this.y > 380) {
            this.y = 380;
        }
        if (this.x > 400) {
        ...
        if (this.y < 0) {
            ...
            if (gameScore === 10 && lives > 0) {
                confirm('Congratulations! Well done');
                lives = 3;
                gameScore = 0;
                livesLeft.innerText = lives;
                score.innerText = '';
